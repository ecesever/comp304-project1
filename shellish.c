...
int process_command(struct command_t *command) {
  ...
  if (pid == 0) // child
  {
   ...
   // exit(127);
	...
	char *path_parts = strtok(pathcopy, ":");
	while (path_parts != NULL){
		//execv function basically kills the child process
		//execv takes 2 params: path pointer and argv[] pointer (args yani)
		//we have args, now we are trying to get the path to put in exec
		char fullpath[1024];
		strcpy(fullpath, path_parts);//copying in order to make a safe work
		strcat(fullpath, "/");
		strcat(fullpath, command->name);//I used strcat to add a string to string to create path
		if (access(fullpath, X_OK) == 0){//the loop checks if there is an access, if yes then ends the loop
			execv(fullpath, command->args);}//execv kills the process
		path_parts = strtok(NULL, ":");//then we keep iterating the next string
	}
	free(pathcopy);
